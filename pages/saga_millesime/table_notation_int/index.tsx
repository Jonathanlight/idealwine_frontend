...
import styles from "../table_notation/index.module.scss";

const Page = () => {
  const { t } = useTranslation("saga-millesime");

  const { data: vintageSagaData } = useQuery(getVintageSagaCountriesQueryParams());

  return (
    <div className={styles.mainContainer}>
      <VintageSagaTableBreadcrumb
        translatedLinkName={"VINTAGE_TABLE_NOTATION_INT_URL"}
        vintageSagaTab={t("internationalWines")}
      />
      <h1 className={styles.mainTitle}>{t("vintageNotes")}</h1>
      <div className={styles.header}>
        <h2 className={styles.tableTitle}>{t("countriesTableTitle")}</h2>
        <TranslatableLink className={styles.link} href={"VINTAGE_TABLE_NOTATION_URL"}>
          <Button variant="primaryGolden">{t("frenchPageButton")}</Button>
        </TranslatableLink>
      </div>
...
const getVintageSagaCountriesQueryParams = () => ({
  staleTime: STALE_TIME_HOUR,
  queryKey: getGetTableVintageSagaCollectionQueryKey({ vintage_saga_countries: true }),
  queryFn: () => getTableVintageSagaCollection({ vintage_saga_countries: true }),
});

const cacheVintageSagaCountries = async (queryClient: QueryClient) => {
  try {
    await queryClient.fetchQuery(getVintageSagaCountriesQueryParams());
  } catch (error) {
...
const getStaticPageProps: DecoratedGetStaticProps = async ({ queryClient }) => {
  await cacheVintageSagaCountries(queryClient);
